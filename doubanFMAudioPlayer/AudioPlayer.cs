...
namespace doubanFMAudioPlayer
{
    public class AudioPlayer : AudioPlayerAgent
    {
        ...
        {
            isLoaded = false;
            CurrentUser = Settings.Get<User>("user");
        ...
        protected async override void OnUserAction(BackgroundAudioPlayer player, AudioTrack track, UserAction action, object param)
        {
            ...
            //{
            //    throw new InvalidOperationException("CurrentUser or CurrnetChannel has not yet been set");
            //}
            switch (action)
            {
                case UserAction.Play:
                    if (player.PlayerState != PlayState.Playing)
                    {
                        if(reset)
                        {
                            reset = false;
                            isLoaded = false;
                            CurrentChannel = Settings.Get<Channel>("currentChannel");
                            player.Track = await GetNextTrack();
                        }
                        player.Play();
        ...
        private async Task<AudioTrack> GetNextTrack()
        {
            ...
            {
                isLoaded = true;
                songs = new List<Song>((await Douban.GetSongs(CurrentUser, CurrentChannel.id.ToString())));
            }
            ...
            AudioTrack track = new AudioTrack(new Uri(CurrentSong.url), CurrentSong.title, CurrentSong.artist, CurrentSong.albumtitle, new Uri(CurrentSong.picture)
                ,(await JsonConvert.SerializeObjectAsync(CurrentSong)), EnabledPlayerControls.SkipNext | EnabledPlayerControls.Pause);
            return track;
        }
        ...
        {
            //never gets called, this function is not implemented for this app.
